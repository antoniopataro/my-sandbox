...
function Drivers({ driverKey }: DriversProps) {
  const navigate = useNavigate();
  const { state } = useLocation();

  const { event }: EventProps | any = state;

  const { cars, handleDeleteCar } = useContext(CarsContext);
  const { setMessage, setToast } = useContext(ToastContext);

  const filteredCars = cars.filter((car) => car.driver["@key"] === driverKey);

  const deleteCar = async (id: number) => {
    const response = await handleDeleteCar(id);

    if (response === "") {
      setMessage("ERROR: Could not delete car");
      setToast(true);
      return;
    }
  ...
  return (
    <DriverStyles>
      {filteredCars.length === 0 ? (
        <h2>
          <span style={{ fontSize: "20px" }}>🚗</span> There are no cars related to this driver.
        </h2>
      ) : (
        <h2>
          <span style={{ fontSize: "20px" }}>🚗</span> Cars:
        </h2>
      )}
      <ul>
        {filteredCars.map((car) => (
          <li key={car.id}>
            ...
            <button onClick={() => navigate(`/${event["@key"]}/cars/edit/${car["@key"]}`, { state: car })}>
