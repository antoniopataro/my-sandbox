...
function Events() {
  ...
  const { events, handleDeleteEvent } = useContext(EventsContext);

  const deleteEvent = async (name: string, date: Date) => {
    const response = await handleDeleteEvent(name, date);
  ...
  return (
    <EventsStyles>
      <ul>
        ...
        {events.map((event) => (
          <li key={event["@key"]}>
            <div className="event-wrapper" onClick={() => navigate(`/${event["@key"]}/cars/`, { state: event })}>
              <h2>{event.name}</h2>
              <h2 className="secondary-card-info">${event.prize}</h2>
            </div>
            ...
            <button onClick={() => deleteEvent(event.name, event.date)}>
