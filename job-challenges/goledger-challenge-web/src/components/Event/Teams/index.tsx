...
function Teams({ event }: TeamsProps) {
  const navigate = useNavigate();

  const { setMessage, setToast } = useContext(ToastContext);
  const { teams, handleDeleteTeam } = useContext(TeamsContext);

  const deleteTeam = async (id: number) => {
    const response = await handleDeleteTeam(id);

    if (response === "failed to delete asset: another asset holds a reference to this one") {
      setMessage("ERROR: There are drivers related to this team");
      setToast(true);
      return;
    }
        ...
        {teams.map((team) => (
          <li key={team.id}>
            <div
              ...
              onClick={() => navigate(`/team/${team.id}`, { state: { event: event, team: team } })}
            >
              {team["@key"] === event.winner["@key"] && <div className="winner-indicator" />}
              <h2>{team.name}</h2>
            </div>
            <button onClick={() => navigate(`/${event["@key"]}/teams/edit/${team.id}`, { state: team })}>
