...
function Cars({ event }: CarsProps) {
  ...
  const handleDriver = (carDriverKey: string) => {
    const driver = drivers.find((driver) => driver["@key"] === carDriverKey);
        ...
        {cars.map((car) => (
          <li key={car["@key"]}>
            <div className="car-wrapper">
              <h2>{car.model}</h2>
              <h2 className="secondary-card-info">{handleDriver(car.driver["@key"])}</h2>
            </div>
